...
OQS_KEM *OQS_KEM_kyber_1024_new(void)
{
	...
	kem->ind_cca = true;
	kem->length_plaintext = OQS_KEM_kyber_1024_length_plaintext;
...
OQS_API OQS_STATUS OQS_KEM_kyber_1024_encaps(uint8_t *ciphertext, uint8_t *shared_secret, const uint8_t *public_key, const uint8_t *message)
{
#if defined(OQS_ENABLE_KEM_kyber_1024_avx2)
#if defined(OQS_DIST_BUILD)
	if (OQS_CPU_has_extension(OQS_CPU_EXT_AVX2) && OQS_CPU_has_extension(OQS_CPU_EXT_BMI2) && OQS_CPU_has_extension(OQS_CPU_EXT_POPCNT))
	{
#endif /* OQS_DIST_BUILD */
		return (OQS_STATUS)pqcrystals_kyber1024_avx2_enc(ciphertext, shared_secret, public_key, message);
#if defined(OQS_DIST_BUILD)
	}
	else
	{
