...
OQS_KEM *OQS_KEM_classic_mceliece_6960119_new(void)
{
	...
	kem->length_plaintext = OQS_KEM_classic_mceliece_6960119_length_plaintext;
...
extern int PQCLEAN_MCELIECE6960119_AVX2_crypto_kem_enc(uint8_t *ct, uint8_t *ss, const uint8_t *pk, const uint8_t *message);
extern int PQCLEAN_MCELIECE6960119_AVX2_crypto_kem_dec(uint8_t *ss, const uint8_t *ct, const uint8_t *sk, const uint8_t *message);
#endif
...
OQS_API OQS_STATUS OQS_KEM_classic_mceliece_6960119_encaps(uint8_t *ciphertext, uint8_t *shared_secret, const uint8_t *public_key, const uint8_t *message)
{
	...
	{
#endif /* OQS_DIST_BUILD */
		return (OQS_STATUS)PQCLEAN_MCELIECE6960119_AVX2_crypto_kem_enc(ciphertext, shared_secret, public_key, message);
#if defined(OQS_DIST_BUILD)
	}
	else
	{
...
OQS_API OQS_STATUS OQS_KEM_classic_mceliece_6960119_decaps(uint8_t *shared_secret, const uint8_t *ciphertext, const uint8_t *secret_key, uint8_t *message)
{
	...
	{
#endif /* OQS_DIST_BUILD */
		return (OQS_STATUS)PQCLEAN_MCELIECE6960119_AVX2_crypto_kem_dec(shared_secret, ciphertext, secret_key, message);
#if defined(OQS_DIST_BUILD)
	}
	else
	{
